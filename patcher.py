...
def main():
    ...
    def enter_recording_mode():
        ...
        try:
            ...
            logging.exception('overlay.update_control_handles failed')
        ...
        # Ensure toolbar stays on top
            ...
            toolbar.setWindowFlag(flag, True)
        toolbar.show()
        ...
        try:
            if getattr(recorder, '_thread', None) and recorder._thread.is_alive():
                ...
                if overlay.isVisible() and (sel is None or sel.isNull()):
                    try:
                        dlg = QtWidgets.QMessageBox(overlay)
                        dlg.setIcon(QtWidgets.QMessageBox.Information)
                        dlg.setWindowTitle('提示')
                        dlg.setText('请先按 Start 键开始录制')
                        dlg.setStandardButtons(QtWidgets.QMessageBox.Ok)
                        dlg.setWindowModality(QtCore.Qt.ApplicationModal)
                        flags = dlg.windowFlags() | QtCore.Qt.WindowStaysOnTopHint
